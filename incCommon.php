	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'books' => array('Books', '', 'resources/table_icons/books.png', 'None'),
			'NewsPapers' => array('NewsPapers', '', 'resources/table_icons/curriculum_vitae.png', 'None'),
			'Magazines' => array('Magazines', '', 'resources/table_icons/blogs.png', 'None'),
			'Users' => array('Users', '', 'resources/table_icons/group.png', 'None'),
			'Book_Issue' => array('Issued', '', 'resources/table_icons/application_from_storage.png', 'None'),
			'Return_Book' => array('Returned', '', 'resources/table_icons/application_side_expand.png', 'None'),
			'Types' => array('Types', '', 'resources/table_icons/chart_organisation.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			...
			'NewsPapers' => "`NewsPapers`.`id` as 'id', `NewsPapers`.`Language` as 'Language', `NewsPapers`.`Name` as 'Name', if(`NewsPapers`.`Data_de_Publicação`,date_format(`NewsPapers`.`Data_de_Publicação`,'%d/%m/%Y'),'') as 'Data_de_Publicação', if(`NewsPapers`.`Data_de_Recebimento`,date_format(`NewsPapers`.`Data_de_Recebimento`,'%d/%m/%Y'),'') as 'Data_de_Recebimento', `NewsPapers`.`Prateleira` as 'Prateleira', `NewsPapers`.`Estante` as 'Estante', `NewsPapers`.`Type` as 'Type', `NewsPapers`.`Editora` as 'Editora'",
			'Magazines' => "`Magazines`.`id` as 'id', `Magazines`.`Type` as 'Type', `Magazines`.`Name` as 'Name', if(`Magazines`.`Data_de_Publicação`,date_format(`Magazines`.`Data_de_Publicação`,'%d/%m/%Y'),'') as 'Data_de_Publicação', if(`Magazines`.`Data_de_Recebimento`,date_format(`Magazines`.`Data_de_Recebimento`,'%d/%m/%Y'),'') as 'Data_de_Recebimento', `Magazines`.`Prateleira` as 'Prateleira', `Magazines`.`Estante` as 'Estante', `Magazines`.`Editora` as 'Editora'",
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'Magazines' => "`Magazines` ",
			'Users' => "`Users` ",
			'Book_Issue' => "`Book_Issue` LEFT JOIN `Users` as Users1 ON `Users1`.`id`=`Book_Issue`.`Member` LEFT JOIN `books` as books1 ON `books1`.`id`=`Book_Issue`.`Book_Number` ",
			'Return_Book' => "`Return_Book` LEFT JOIN `Book_Issue` as Book_Issue1 ON `Book_Issue1`.`id`=`Return_Book`.`Book_Number` LEFT JOIN `books` as books1 ON `books1`.`id`=`Book_Issue1`.`Book_Number` LEFT JOIN `Users` as Users1 ON `Users1`.`id`=`Return_Book`.`Member` ",
			'Types' => "`Types` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('SAIDE_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('SAIDE_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['SAIDE_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['SAIDE_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> SAIDE</a>
			</div>
